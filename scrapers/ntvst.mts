...
//: ntv.st's own API rate-limits aggressively once the channel index has
//: been paged through a few thousand times in a row -- retrying with a
//: short backoff clears it in practice; it is not a sign the request
//: itself was wrong.
...
async function fetchAllChannels(): Promise<NtvChannel[]> {
...
async function buildChannels(): Promise<ScrapedChannel[]> {
...
async function build(): Promise<ScrapedCatalogue> {
    const [channels, events] = await Promise.all([buildChannels(), buildEventsRail()]);
